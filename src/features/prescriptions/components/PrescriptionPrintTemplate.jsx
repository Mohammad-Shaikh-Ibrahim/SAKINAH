...
export const PrescriptionPrintTemplate = React.forwardRef(({ prescription, patient, doctor }, ref) => {
    ...
    return (
        <Box ref={ref} sx={{ p: 5, bgcolor: 'white', color: 'black', printColorAdjust: 'exact', width: '100%', maxWidth: '210mm', minHeight: '297mm', margin: '0 auto' }}>
            {/* Header */}
            ...
            <Grid container spacing={2} sx={{ mb: 4, border: '1px solid #eee', p: 2, borderRadius: 1 }}>
                <Grid item xs={6}>
                    ...
                    <Typography variant="body1" fontWeight="bold">{prescription.patientName || `${patient?.firstName} ${patient?.lastName}`}</Typography>
                </Grid>
                <Grid item xs={3}>
                    <Typography variant="caption" color="text.secondary">DATE</Typography>
                    <Typography variant="body1">{format(new Date(prescription.prescriptionDate), 'MMMM dd, yyyy')}</Typography>
                </Grid>
                <Grid item xs={3}>
                    <Typography variant="caption" color="text.secondary">RX ID</Typography>
                    <Typography variant="body1" sx={{ fontFamily: 'monospace' }}>#{prescription.id.slice(0, 8)}</Typography>
                </Grid>
            </Grid>
            ...
            <Box sx={{ minHeight: '400px' }}>
                ...
                {prescription.medications.map((med, index) => (
                    <Box key={med.id} sx={{ mb: 4, borderLeft: '4px solid #eee', pl: 2 }}>
            ...
            <Box sx={{ mt: 'auto', pt: 8 }}>
                ...
                <Box sx={{ mt: 4, pt: 2, borderTop: '1px solid #eee', textAlign: 'center' }}>
                    <Typography variant="caption" color="text.secondary">
                        ...
                        Generated electronically by SAKINAH EMR system on {format(new Date(), 'PPpp')}.
                    </Typography>
                </Box>
            </Box>
        </Box>
